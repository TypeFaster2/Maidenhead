...
v3: Originally based on sketch from the TinyGPS site. 27 Oct 2012
 ...
 v15_Volt_Test: Added volt meter                       12 Dec 2012
 v17_Small: Shrink variable use                        16 Dec 2012
...
#define VoltIn 0                         // Analouge Pin for Volt Meter
...
void setup(){
  ...
  lcd.print(" Amateur Radio    Decoder by            GPS Grid Square  VK3FMAL    V17v");  
#ifdef Demo
  lcd.setCursor(0,0);                            // set the preamble text
...
void loop(){
  ...
  if(CycleTimer > millis()){                       // First cycle (Display 1)
...
#ifdef sprnt 
    Serial.print("First Cycle, Uptime : ");
    ...
    if((CycleTimer + 10000) > millis()){        // This sets how long we show display 2
      ...
      GPS_Time(1);         
#ifdef sprnt 
      Serial.print("Second Cycle, Uptime : ");
      Serial.println(millis() / 1000);
#endif    
      ...
      Serial.print("Reset Cycle, Uptime : ");
      Serial.println(millis() / 1000);
#endif }
    }
...
void VoltMeter(){
  ...
  /*  Reference Voltage    : 5.059v
   Register Incerements : 1023
   Volt divider. R1 10k, R2 3.3k
   Volt Factor = (RefVolt / Register Increment) / (R2/(R2 + R1))
   Volt Factor = 0.01993
   We don't want to use the floating point library, so multiply by 10,000. therefore : 199  
   */
  long A0 = analogRead(A0);                        // Read the voltage sense pin

  long Volts1 = (10000*A0)*199;                // Convert reading to volts.        
  long Volts  = Volts1 / 100000000;            // Whole volts
  long mVolts = ((Volts1 - (Volts * 100000000)) / 100000);  // miliVolts

  int rounder = mVolts % 10;
  if (rounder > 5){
    mVolts = (mVolts / 10) + 1;
  } 
  else {
...
void MaidenHead(){
  ...
  i = 0;                                           // into a string that's easy to print
  while (i < 6){ 
    lcd.print(MH[i]);
    i++; 
  }
...
void GPS_Time(int Time_Type){
  ...
  if(time > 23595959) {                               // Check we have a valid input
    //        1919221760
#ifdef sprnt 
    Serial.print("Time to big. Time_Type is : ");
    Serial.println( Time_Type);                         // Too big, will give weird results
    Serial.print(time);  
  ...
  int *pt_mn = &minute;
  int *pt_sc = &second;
  void strng_time(int *pt_hr, int *pt_mn, int *pt_sc);


  // String Tme = "";
  char sTme[9] = "00:00:00";
  ...
  if(hour < 10){
    sTme[0] = 48;                     // print out the time (GMT)
  ...
  if(Time_Type == 1){
    lcd.print("LclTime ");        // test if this is lcl or gmt time
#ifdef sprnt   
    Serial.print("Lcl Time is: ");

#endif
  } 
  else {
    lcd.setCursor(16, 0);                          // place the cursor for text
    lcd.print("GMT Time ");

    lcd.setCursor(24, 1);                          // place cursor for actual time
#ifdef sprnt      
    Serial.print("GMT Time is : ");
#endif

  }                                              // end of the if statement
  
 // Serial.print(*strng_time[0]);
  /*
  int i = 0;
   while (i < 2){ 
    Serial.print(&strng_time[i]);
    i++; 
   }
 */ 
...
void GPS_Locn(int LatLon){
...
#ifdef Demo
    Serial.print("Lat is : ");
  Serial.println(lat);                              // Demo mode for testing
  Serial.print("Lon is : "):
  Serial.println(lon);
...
#ifdef  sprnt
    Serial.print("******* Locn Error ************************************");
    Serial.println(0);
    Serial.print("LLDec is : ");
    Serial.print(LLDec);
#endif
    return;
  }    
  ...
  else                       // Get spacing in front of Deg right
  {
    if(Deg > 9){
      DMS += " "; 
      DMS += Deg;
  ...
  if(LatLon == 2){                                     // What is it we just worked out? (Lat or Lon)
#ifdef sprnt     
    Serial.print("Latitude   : ");
    Serial.print(DMS);  
#endif
    DMS += " Volt";                                      // Set up for volt readout
...
void GPS_Stats(){
  ...
  if (alt > 9999){
    text += "9999";
  }                         // Make sure we don't really long data
  else {
    if(alt >999){
      text += alt;
    }                        // Would apply before a good reading
    else {
      if(alt > 99){
        text += " "  ; 
        text += alt;
  ...
  lcd.setCursor(0, 0); 
  lcd.print(text) ;                   // Alt & Sats in one string
  ...
  if (failed_checksum > 999){
    text += "999";
  }
  else {
    if(failed_checksum > 99){
      text += failed_checksum;
    }
    else {
      if(failed_checksum >  9){
        text += " " ; 
        text += failed_checksum;
      } 
      else              {
  ...
  lcd.setCursor(0, 1); 
  lcd.print(text) ;                   // HDOP & Checksum in one string
...
void GPS_FixAge(){
  ...
  if (fix_age == TinyGPS::GPS_INVALID_AGE)
    ...
    Serial.print("Data is current. Fix Age: ");
  Serial.println(fix_age);
#endif  

  if (fix_age > 600000){
    fix_age = 0000;
  }      // Try to stop the invalid fix age overflowing LCD

  String text = "Age ";                       // Leading text to the string
  if(fix_age > 9999){
    text += "9999";
  }       // Keep the 1's unit in the same place on the LCD
  else {
    if (fix_age > 999){
      text += fix_age;
    }
...
int availableMemory() {
  ...
  int size = 2048;                        // Use 1024 with ATmega168
  byte *buf;
